...
export function matchingClaims(node, { confidence = "", deviceReview = "" } = {}) {
  return (node.quality_claims ?? []).filter((claim) => {
    ...
    if (["required", "recommended"].includes(deviceReview))
      return claim.device_requirement === deviceReview && claim.device_state === "pending";
    return !deviceReview || claim.device_state === deviceReview;
  });
}

export function reviewSelection(model, matchedIds, filters, limit = null) {
  const claims = [...matchedIds].flatMap((id) => matchingClaims(model.byId.get(id), filters));
  claims.sort((a, b) => a.priority - b.priority || a.feature_id.localeCompare(b.feature_id, "en") || a.claim_id.localeCompare(b.claim_id, "en"));
  ...
  return {
    ...
    claims: selected,
    notice: "仅导出真正匹配的知识项，不包括用于导航的祖先。未启动研究或真机测试；样本通过不能外推全部机型。",
  };
}

export function downloadReviewScope(scope) {
  const url = URL.createObjectURL(new Blob([JSON.stringify(scope, null, 2)], { type: "application/json" }));
