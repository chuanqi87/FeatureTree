...
To enable shader logging, you first need to enable the compiler setting in [`MTLCompileOptions`](/documentation/Metal/MTLCompileOptions). You can either do this directly through Xcode, or while building your [`MTLLibrary`](/documentation/Metal/MTLLibrary). Shader logging is only available in Metal 3.2 and later. It’s also only available in iOS 18 and later, iPadOS 18 and later, macOS 15 and later, tvOS 18 and later, and visionOS 2 and later.
...
If you’re creating a Metal Library by compiling source code using `newLibraryWithSource`, use the `MTLCompileOptions` instance’s [`enableLogging`](/documentation/Metal/MTLCompileOptions/enableLogging) property to set the compile option:
...
The above methods enable shader logging for the entire app. However, if you want to enable logging for a specific command buffer or command queue, you can use the Metal API, [`MTLLogState`](/documentation/Metal/MTLLogState).

To create an `MTLLogState`, first create an [`MTLLogStateDescriptor`](/documentation/Metal/MTLLogStateDescriptor), and set its buffer size and log level:
...
By default, you can still view all log messages through either the Console app or the `log` command-line tool. However, log handlers give you an alternative method to preview the logs. The [`addLogHandler(_:)`](/documentation/Metal/MTLLogState/addLogHandler(_:)) method allows you to add one or more log handlers to a log state. You can also filter and customize the way you preview the messages by using a subsystem and category:
