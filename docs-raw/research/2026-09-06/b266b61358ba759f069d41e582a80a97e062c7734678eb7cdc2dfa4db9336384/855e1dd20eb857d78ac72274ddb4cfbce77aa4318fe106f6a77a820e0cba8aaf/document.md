...
![](https://media:201787906583666256)  
...
|fds^15+^|Record\<string, number\>|是|是|表示文件描述符的集合。在应用启动场景中，拉起方通过[startAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startability)传递Want时，需在parameters中以固定键值对形式传入文件描述符，被拉起方可通过该字段获取文件描述符，具体使用方式见"文件描述符（FD）"示例。 元服务API：从API version 15开始，该接口支持在元服务中使用。|
