...
![](https://media:201787906774096061)  
...
|callback|Callback\<{ reason: number; }\>|是|注册设备发现失败的回调方法，返回设备发现失败的原因。回调参数reason为数值类型，表示发现失败的错误原因码，具体取值可参见[设备管理错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-device-manager)。|
