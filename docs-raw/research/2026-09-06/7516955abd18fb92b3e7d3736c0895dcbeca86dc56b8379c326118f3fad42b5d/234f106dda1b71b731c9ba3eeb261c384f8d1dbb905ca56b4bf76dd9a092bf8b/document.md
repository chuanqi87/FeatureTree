...
MDM应用进程模型继承于普通应用[进程模型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/process-model-overview)，在普通应用模型基础上MDM应用会多一个独立的EnterpriseAdmin进程，MDM应用的Admin组件被激活后，EnterpriseAdmin进程会被创建，EnterpriseAdmin进程作为设备管理应用的后台进程，用于接收MDM应用的激活、取消激活等事件的回调。EnterpriseAdmin进程的生命周期不受到主进程的影响，由系统管理其生命周期。Admin组件的激活方式不同，EnterpriseAdmin进程的生命周期的[管理方式](#admin组件激活规格的差异)也不同。

图1 MDM应用进程模型

![](https://media:201787906114862857)  
...
![](https://media:201787906114898858)

图3 存在MDM应用的前台进程和EnterpriseAdmin进程

![](https://media:201787906114928859)

图4 应用主进程停止时，EnterpriseAdmin进程仍然运行

![](https://media:201787906114957860)

图5 EnterpriseAdmin进程支持系统事件回调

![](https://media:201787906114988861)
...
![](https://media:201787906115032862)  
...
![](https://media:201787906115065863)
