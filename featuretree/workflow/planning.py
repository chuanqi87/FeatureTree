...
def make_plan(root, nodes, baseline, depth=2, max_nodes=40, workers=3,
              timeout=600, max_attempts=2, max_revisions=2, model=None, variant=None,
              max_input_chars=500000):
    if not (1 <= depth <= 10 and 1 <= max_nodes <= 200 and 1 <= workers <= 16
            and 1 <= timeout <= 7200 and 1 <= max_attempts <= 5 and 0 <= max_revisions <= 5
            and 10000 <= max_input_chars <= 2000000):
    ...
    for node in nodes:
        for stage in STAGES:
            deps = [] if stage == "scope" else [f"{node}/scope"] if stage in PLATFORMS else (
