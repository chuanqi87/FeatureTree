...
In the Network framework, a bidirectional flow of data is represented as a connection object ([`nw_connection_t`](doc://com.apple.documentation/documentation/Network/nw_connection_t)).  If you’re using TCP, there’s a direct mapping between the connection object and the underlying TCP connection. If you’re using UDP, a connection object represents a bidirectional flow of datagrams between a local port and a port on a specific remote peer.

To create an outbound connection object, you must supply a parameters object ([`nw_parameters_t`](doc://com.apple.documentation/documentation/Network/nw_parameters_t)) and an endpoint ([`nw_endpoint_type_t`](doc://com.apple.documentation/documentation/Network/nw_endpoint_type_t)).

**Create a Parameters Object.** A parameters object, of type [`nw_parameters_t`](doc://com.apple.documentation/documentation/Network/nw_parameters_t), holds all the parameters necessary to configure a network connection.  These include:
...
To create a parameters object, call either the [`nw_parameters_create_secure_tcp(_:_:)`](/documentation/Network/nw_parameters_create_secure_tcp(_:_:)) or the [`nw_parameters_create_secure_udp(_:_:)`](/documentation/Network/nw_parameters_create_secure_udp(_:_:)) convenience function, depending on whether you want to use TCP or UDP.
...
**Create an Endpoint.** An endpoint, of type [`nw_endpoint_type_t`](doc://com.apple.documentation/documentation/Network/nw_endpoint_type_t), holds a network host or service name.  For an outbound connection, the endpoint determines the remote host to which you want to connect.  In most cases this consists of a host name and a port number, but there are other options.  For example, you can also create endpoints that target a Bonjour service.

In the `nwcat` tool, the user supplies a host name and a port number via command-line arguments, and you will need to create an endpoint from those two strings.  You do this by calling [`nw_endpoint_create_host(_:_:)`](/documentation/Network/nw_endpoint_create_host(_:_:)).
...
> Important: If you pass in a DNS name, the connection object takes care of DNS resolution for you, handling the complex issue of [deciding what IP address to connect to](https://tools.ietf.org/html/rfc8305).  For outbound connections there’s no need to do your own DNS resolution, and in most cases doing so will result in a worse user experience.

**Create the Connection Object.**  Once you have your parameters object and endpoint, you can create a connection object by calling [`nw_connection_create(_:_:)`](/documentation/Network/nw_connection_create(_:_:)).
...
1. Call [`nw_connection_set_queue(_:_:)`](/documentation/Network/nw_connection_set_queue(_:_:)) to set the [`dispatch_queue_t`](doc://com.apple.documentation/documentation/Dispatch/dispatch_queue_t) on which all callbacks will be scheduled.  For a simple application like `nwcat`, you can use the main queue for your callbacks.  A more complex application would typically use a custom serial queue instead.
2. Install any update handler blocks. The most important of these is the state changed handler, discussed below.
3. Start the connection by calling [`nw_connection_start(_:)`](/documentation/Network/nw_connection_start(_:)).
...
> Important: The connection object closes the underlying network connection when its last reference is released.  Thus, you must retain the connection object, by calling ``doc://com.apple.network/documentation/Network/nw_retain``, until you’re done with the connection.
...
Make sure you handle the [`nw_connection_state_cancelled`](doc://com.apple.documentation/documentation/Network/nw_connection_state_cancelled) state.  Once the connection is no longer needed, you have to release the reference you took when you started the connection.

### Listen for an Inbound Connection

A listener object, of type [`nw_listener_t`](doc://com.apple.documentation/documentation/Network/nw_listener_t), listens for inbound connections and creates a new connection object for each one.  To create a listener object you must supply a parameters object ([`nw_parameters_t`](doc://com.apple.documentation/documentation/Network/nw_parameters_t)) to indicate what protocols to use and information about the local endpoint on which you want to listen.

**Create a Parameters Object.** Creating a parameters object for a listener object is very similar to creating a parameters object for an outbound connection. Use the [`nw_parameters_create_secure_tcp(_:_:)`](/documentation/Network/nw_parameters_create_secure_tcp(_:_:)) and [`nw_parameters_create_secure_udp(_:_:)`](/documentation/Network/nw_parameters_create_secure_udp(_:_:)) convenience functions to define which protocols you want your listener to use. These parameters will be applied to any inbound connections your listener accepts. For example, if you enable TLS in the parameters object, all inbound connections will negotiate TLS once you call [`nw_connection_start(_:)`](/documentation/Network/nw_connection_start(_:)).
...
Most applications don’t need to listen on a specific interface address and thus can create a listener using the [`nw_listener_create_with_port(_:_:)`](/documentation/Network/nw_listener_create_with_port(_:_:)) convenience function.  However, `nwcat` allows the user to specify an interface address (via a command line argument) and thus you have to use a slightly more complex technique.  If the user has specified an interface address or a port, you can call [`nw_endpoint_create_host(_:_:)`](/documentation/Network/nw_endpoint_create_host(_:_:)) to create an endpoint representing the address to listen on, and then call [`nw_parameters_set_local_endpoint(_:_:)`](/documentation/Network/nw_parameters_set_local_endpoint(_:_:)) to apply that to your parameters object.
...
When calling [`nw_endpoint_create_host(_:_:)`](/documentation/Network/nw_endpoint_create_host(_:_:)) to create an endpoint for the local address, keep the following in mind:
...
**Create the Listener Object.** Once you’ve set up your parameters object, you can create a listener object by calling [`nw_listener_create(_:)`](/documentation/Network/nw_listener_create(_:)).
...
When the receive is complete, [`nw_connection_receive(_:_:_:_:)`](/documentation/Network/nw_connection_receive(_:_:_:_:)) calls the completion handler that you pass it.  The completion handler has four parameters:

- A [`dispatch_data_t`](doc://com.apple.documentation/documentation/Dispatch/dispatch_data_t) which, if not `NULL`, contains the data received.
...
A content context, of type [`nw_content_context_t`](doc://com.apple.documentation/documentation/Network/nw_content_context_t), holds extra information about the data received.  A typical application that uses only TCP can often ignore this value entirely.  However, a `netcat` implementation must work equally well with TCP and UDP, and you need the content context to do that.

The completion handler you pass to [`nw_connection_receive(_:_:_:_:)`](/documentation/Network/nw_connection_receive(_:_:_:_:)) does the following:
