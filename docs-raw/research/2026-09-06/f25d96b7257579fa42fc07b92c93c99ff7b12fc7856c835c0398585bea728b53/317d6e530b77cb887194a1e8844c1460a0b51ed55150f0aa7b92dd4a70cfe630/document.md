...
In iOS 14 and watchOS 8 and later, devices equipped with the first-generation UWB chip are capable of precise distance measurement through the Nearby Interaction framework. Devices supporting <doc://com.apple.documentation/documentation/ARKit> can locate stationary objects or devices. Devices equipped with the second-generation UWB chip are capable of EDM. Pairs of devices supporting both extended distance measurement and Camera Assistance have the ability to locate moving devices. You can query these capabilities through the [`deviceCapabilities`](/documentation/NearbyInteraction/NISession/deviceCapabilities) property check.
...
To determine if a peer device supports EDM, examine the peer’s [`NIDiscoveryToken`](/documentation/NearbyInteraction/NIDiscoveryToken):
...
If both sides support EDM, they can run [`NISession`](/documentation/NearbyInteraction/NISession) with the configuration that has EDM enabled, as shown in the example below:
