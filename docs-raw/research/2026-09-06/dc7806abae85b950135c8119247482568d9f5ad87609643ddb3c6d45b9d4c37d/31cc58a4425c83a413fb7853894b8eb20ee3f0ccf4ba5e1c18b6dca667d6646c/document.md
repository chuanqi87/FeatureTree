...
GATT是低功耗蓝牙（BLE）的核心协议，定义了基于服务（Service）、特征值（Characteristic）和描述符（Descriptor）进行蓝牙通信和传输数据的机制。相关术语介绍请参考[蓝牙术语](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/terminology)。  
