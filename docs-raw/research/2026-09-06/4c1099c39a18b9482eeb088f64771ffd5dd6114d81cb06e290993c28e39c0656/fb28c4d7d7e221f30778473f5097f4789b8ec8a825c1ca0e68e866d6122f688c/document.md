...
![](https://media:201787906363256623)  
...
![](https://media:201787906363285624)  
...
|名称|类型|常量值|说明|
|:---------------|:-----|:--|:-----------|
|MAX_TYPE_NUM^8+^|number|128|可支持的最大输入法个数。|

#### InputMethodProperty^8+^
...
|名称|类型|只读|可选|说明|
|:------------------------|:------------------------------|:-|:-|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|name^9+^|string|是|否|必填。输入法包名。|
|id^9+^|string|是|否|必填。输入法扩展在应用内唯一标识，与name一起组成输入法扩展的全局唯一标识。|
|label^9+^|string|是|是|非必填。 - 当InputMethodProperty用于切换、查询等接口的入参时，开发者可不填写此字段，通过name和id即可唯一指定一个输入法扩展。 - 当InputMethodProperty作为查询接口的返回值时（如[getCurrentInputMethod](#inputmethodgetcurrentinputmethod9)），此字段表示输入法扩展对外显示的名称，优先使用InputMethodExtensionAbility中配置的label，若未配置，自动使用应用入口ability的label；当应用入口ability未配置label时，自动使用应用AppScope中配置的label。|
|labelId^10+^|number|是|是|非必填。 - 当InputMethodProperty用于切换、查询等接口的入参时，开发者可不填写此字段，通过name和id即可唯一指定一个输入法扩展。 - 当InputMethodProperty作为查询接口的返回值时（如[getCurrentInputMethod](#inputmethodgetcurrentinputmethod9)），此字段表示label字段的资源号。|
|icon^9+^|string|是|是|非必填。 - 当InputMethodProperty用于切换、查询等接口的入参时，开发者可不填写此字段，通过name和id即可唯一指定一个输入法扩展。 - 当InputMethodProperty作为查询接口的返回值时（如[getCurrentInputMethod](#inputmethodgetcurrentinputmethod9)），此字段表示输入法图标数据，可以通过iconId查询获取。|
|iconId^9+^|number|是|是|非必填。 - 当InputMethodProperty用于切换、查询等接口的入参时，开发者可不填写此字段，通过name和id即可唯一指定一个输入法扩展。 - 当InputMethodProperty作为查询接口的返回值时（如[getCurrentInputMethod](#inputmethodgetcurrentinputmethod9)），此字段表示icon字段的资源号。|
|enabledState^20+^|[EnabledState](#enabledstate15)|是|是|非必填。 - 当InputMethodProperty用于切换、查询等接口的入参时，开发者可不填写此字段，通过name和id即可唯一指定一个输入法扩展 - 当InputMethodProperty作为查询接口的返回值时（如[getCurrentInputMethod](#inputmethodgetcurrentinputmethod9)），此字段表示该输入法启用状态。|
|extra^9+^|object|否|是|输入法扩展信息。 - API version 10起：非必填； - API version 9：必填。|
...
#### inputMethod.switchInputMethod^9+^
...
![](https://media:201787906363336625)  
...
![](https://media:201787906363358626)  
在 API11 中 201 permissions check fails. 这个错误码被移除。  
...
#### inputMethod.switchCurrentInputMethodSubtype^9+^
...
![](https://media:201787906363387627)  
在 API11 中 201 permissions check fails. 这个错误码被移除。  
...
![](https://media:201787906363410628)  
在 API11 中 201 permissions check fails. 这个错误码被移除。  

#### inputMethod.getCurrentInputMethodSubtype^9+^
...
#### inputMethod.switchCurrentInputMethodAndSubtype^9+^
...
![](https://media:201787906363435629)  
在 API11 中 201 permissions check fails. 这个错误码被移除。  
...
![](https://media:201787906363457630)  
在 API11 中 201 permissions check fails. 这个错误码被移除。  
...
获取客户端实例[InputMethodController](#inputmethodcontroller)。  
![](https://media:201787906363485631)  
...
获取客户端设置实例[InputMethodSetting](#inputmethodsetting8)。  
![](https://media:201787906363585632)  
...
#### inputMethod.offAttachmentDidFail^22+^
...
|NUMBER_PASSWORD^11+^|8|数字密码类型。 使用场景：适用于仅需输入数字密码的场景，如PIN码输入。|
|SCREEN_LOCK_PASSWORD^20+^|9|锁屏密码类型。 使用场景：适用于锁屏界面的密码输入框。|
|USER_NAME^20+^|10|用户名类型。 使用场景：适用于用户名输入框，输入法可根据用户名特点优化建议。|
|NEW_PASSWORD^20+^|11|新密码类型。 使用场景：适用于设置新密码的输入框，输入法可提供密码强度提示。|
|NUMBER_DECIMAL^20+^|12|带小数点的数字类型。 使用场景：适用于需要输入带小数点数字的场景，如金额输入。|
|ONE_TIME_CODE^20+^|13|验证码类型。 使用场景：适用于验证码输入框，输入法可优化验证码输入体验。|
...
|NEWLINE^12+^|8|换行。 使用场景：适用于多行文本编辑框，Enter键插入换行符。|
...
|placeholder^20+^|string|否|是|编辑框设置的占位符信息。 - 编辑框设置占位符信息时，长度不超过255个字符（如果超出将会自动截断为255个字符），用于提示或引导用户输入临时性文本或符号。（例如：提示输入项为"必填"或"非必填"的输入结果反馈。） - 编辑框没有设置占位符信息时，默认为空字符串。 - 该字段在调用[attach](#attach10)时提供给输入法应用。|
|abilityName^20+^|string|否|是|编辑框设置的ability名称。 - 编辑框设置ability名称时，长度不超过127个字符（如果超出将会自动截断为127个字符）。 - 编辑框未设置ability名称时，默认为空字符串。 - 该字段在调用绑定[attach](#attach10)时提供给输入法应用。|
...
|newEditBox^20+^|boolean|否|是|表示是否为新编辑框。true表示新编辑框，false表示非新编辑框。|
|capitalizeMode^20+^|[CapitalizeMode](#capitalizemode20)|否|是|编辑框设置大小写模式。如果没有设置或设置非法值，默认不进行任何首字母大写处理。|
...
|displayId^23+^|number|否|是|输入法软键盘窗口所在的屏幕ID。 模型约束： 该参数仅可在Stage模型下使用。|
...
自定义通信对象。  
![](https://media:201787906363607633)  
...
接收输入法应用发送的自定义数据回调函数。  
![](https://media:201787906363631634)  
...
监听对象终止回调函数。  
![](https://media:201787906363654635)  
...
![](https://media:201787906363685636)  
...
![](https://media:201787906363709637)  
...
![](https://media:201787906363734638)  
需要先调用此接口，完成自绘控件与输入法的绑定，才能使用以下功能：显示/隐藏键盘、更新光标信息、更改编辑框选中范围、保存配置信息、监听处理由输入法应用发送的信息或命令等。
...
自绘控件绑定输入法。使用promise异步回调。  
![](https://media:201787906363758639)  
需要先调用此接口，完成自绘控件与输入法的绑定，才能使用以下功能：显示/隐藏键盘、更新光标信息、更改编辑框选中范围、保存配置信息、监听处理由输入法应用发送的信息或命令等。
...
![](https://media:201787906363839642)  
...
![](https://media:201787906363868643)  
编辑框与输入法绑定成功后，可调用该接口拉起软键盘，进入文本编辑状态。
...
进入文本编辑状态。使用promise异步回调。  
![](https://media:201787906363907644)  
编辑框与输入法绑定成功后，可调用该接口拉起软键盘，进入文本编辑状态。
...
![](https://media:201787906363933645)  
...
![](https://media:201787906363962646)  
调用接口时，若软键盘处于显示状态，调用接口后软键盘会被隐藏。
...
![](https://media:201787906363985647)  
...
![](https://media:201787906364016648)  
将绑定到输入法的应用程序所在的窗口Id传入，此窗口可以避让输入法窗口。
...
![](https://media:201787906364047649)  
...
![](https://media:201787906364070650)  
编辑框与输入法绑定成功后，才可调用该接口更新光标信息。
...
![](https://media:201787906364297651)  
...
![](https://media:201787906364326652)  
编辑框与输入法绑定成功后，才可调用该接口更新文本选区信息。
...
![](https://media:201787906364351653)  
...
![](https://media:201787906364376654)  
...
![](https://media:201787906364403655)  
该接口需要编辑框与输入法绑定时才能调用，即点击编辑控件后，才可调用该接口结束输入会话。
...
![](https://media:201787906364426656)  
...
![](https://media:201787906364454657)  
该接口需要编辑框与输入法绑定时才能调用，即点击编辑控件后，才可调用显示当前输入法的软键盘。
...
![](https://media:201787906364483658)  
...
![](https://media:201787906364508659)  
该接口需要编辑框与输入法绑定时才能调用，即点击编辑控件后，才可调用隐藏当前输入法的软键盘。
...
![](https://media:201787906364537660)  
...
![](https://media:201787906364565661)  
...
结束输入会话。使用callback异步回调。  
![](https://media:201787906364590662)  
该接口需要编辑框与输入法绑定时才能调用，即点击编辑控件后，才可调用该接口结束输入会话。
...
结束输入会话。使用promise异步回调。  
![](https://media:201787906364615663)  
该接口需要编辑框与输入法绑定时才能调用，即点击编辑控件后，才可调用该接口结束输入会话。
...
![](https://media:201787906364643664)  
...
![](https://media:201787906364673665)  
...
#### listCurrentInputMethodSubtype^9+^
...
![](https://media:201787906364731666)  
...
![](https://media:201787906364756667)  
已激活输入法为使能的输入法应用。默认输入法默认使能，其他输入法可被设置为使能或非使能。
...
#### getAllInputMethodsSync^11+^

getAllInputMethodsSync(): Array\<InputMethodProperty\>

获取所有输入法应用列表。同步接口。  
![](https://media:201787906364807669)  
同步接口阻塞主线程，容易影响UI交互，需谨慎使用。
...
![](https://media:201787906364839670)  
...
![](https://media:201787906364861671)  
从API version 9开始支持，从API version 18开始废弃，建议使用[InputMethodListDialog](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inputmethodlist#inputmethodlistdialog)替代。
...
![](https://media:201787906364885672)  
...
![](https://media:201787906364915673)  
...
显示输入法选择对话框。使用callback异步回调。  
![](https://media:201787906364940674)  
...
显示输入法选择对话框。使用promise异步回调。  
![](https://media:201787906364965675)  
从API version 8开始支持，从API version 9开始废弃，建议使用[InputMethodListDialog](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inputmethodlist#inputmethodlistdialog)替代。
