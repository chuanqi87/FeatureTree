...
![](https://media:901787638352095205 "点击放大")
...
1. 选择entry模块，暂时关闭模块build-profile中的混淆开关。 ![](https://media:901787638352140206 "点击放大")

2. 检查一下build模式，改为release。 ![](https://media:901787638352173207)

3. 点击菜单 -\> tool -\> AppAnalyzer，打开体检工具。
4. 选择"场景化体检"，点击"手动性能冷启动体检"。 工具开始准备，会自动编译、安装、运行当前工程，此时需要"保持手机解锁状态"，当准备完成后，会提示点击开始按钮，开始体检。

   ![](https://media:901787638352226208 "点击放大")
5. 根据提示在设备上进行手动操作：
   1. 首先，在最近任务列表关闭应用。
...
![](https://media:901787638352251209)  
冷启动体检需要强行停止应用：
...
![](https://media:901787638352303210 "点击放大")  
...
![](https://media:901787638352420211 "点击放大")
...
![](https://media:901787638352463212 "点击放大")

import加载耗时问题优化流程

![](https://media:901787638352669213 "点击放大")
...
4. 判断上层文件是否为为其他module的Index.ets文件导出。
   ...
   * 如果上层文件为其他module的统一对外暴露接口文件（例如Index.ets），则需要通过import路径展开性能优化。 例如CustomLayout1上层模块为library模块的Index.ets，其中有多个文件导出。
     ...
     ![](https://media:901787638352861217 "点击放大")
...
5. 在导出文件中标记已修改的文件。 在下载全量import文件清单表格中，标记已优化的未使用文件，便于优化备忘，特别是当需要优化的文件比较多的时候。
   ...
   ![](https://media:901787638352914218 "点击放大")
...
* 网络请求本身耗时长 ![](https://media:901787638352950219 "点击放大")
...
* 网络请求发起太晚 ![](https://media:901787638352985220 "点击放大")
...
![](https://media:901787638353030221 "点击放大")
...
![](https://media:901787638353077222 "点击放大")
...
![](https://media:901787638353307225 "点击放大")

针对应用冷启动问题的性能分析，可以选择分析主线程的Trace数据或采样得到的函数热点。

分析主线程的Trace数据

![](https://media:901787638353359226 "点击放大")
...
![](https://media:901787638353411227 "点击放大")
...
![](https://media:901787638353466228 "点击放大")
...
![](https://media:901787638353596229 "点击放大")
...
![](https://media:901787638353713232 "点击放大")
...
![](https://media:901787638353753233 "点击放大")
...
![](https://media:901787638353792234 "点击放大")
...
图4 优化网络请求时机前   
![](https://media:901787638353829235 "点击放大")  
图5 优化网络请求时机后   
![](https://media:901787638353902236 "点击放大")
...
![](https://media:901787638353930237)  
...
![](https://media:901787638353957238)  
图7 优化后使用144px-144px启动页图标应用启动耗时   
![](https://media:901787638353983239)
...
图8 优化前import 15个模块   
![](https://media:901787638354019240 "点击放大")  
图9 优化后import 5个模块   
![](https://media:901787638354052241 "点击放大")
...
* 减少使用嵌套的export \*方式进行全量导出 在应用冷启动过程中，\`HandleLaunchAbility\` 会执行冷启动相关的 \`.ets\` 文件。所有被主页面import的 \`.ets\` 文件均会被执行，包括数据结构、变量和全局函数的初始化。首页所需的变量和函数可能来自其他 \`.ets\` 文件，并通过export形式提供给首页使用。详细信息请参见[减少同文件大量export \*导出方式](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkts-high-performance#section1218510102815)。
  ...
  ![](https://media:901787638354084242 "点击放大")  
  图11 （优化后）不存在嵌套export \*，从目标文件中直接import   
  ![](https://media:901787638354123243 "点击放大")
...
* 减少import \*的方式全量引用 应用程序加载时，通常将相同类型的变量或函数放在同一个工具类文件中。使用时，通过import方式引入对应的模块。当工具类中存在较多暴露的函数或变量时，推荐按需引用具体的变量，以减少 \`.ets\` 文件的执行耗时和文件中所有export变量的初始化过程。详情请参见[减少同文件大量export \*导出方式](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkts-high-performance#section1218510102815)。
  ...
  ![](https://media:901787638354156244 "点击放大")  
  图13 优化后，使用import { One }按需引用   
  ![](https://media:901787638354187245 "点击放大")
  ...
  ![](https://media:901787638354220246)  
...
![](https://media:901787638354288247 "点击放大")
...
![](https://media:901787638354360248 "点击放大")
...
![](https://media:901787638354426249 "点击放大")
...
![](https://media:901787638354452250)  
...
![](https://media:901787638354483251 "点击放大")  
图18 优化方案一：拆分HAR导出文件   
![](https://media:901787638354514252 "点击放大")  
图19 优化方案二：导入冷启动文件时全路径展开   
![](https://media:901787638354544253 "点击放大")
...
图20 HAP+20个HSP混合打包   
![](https://media:901787638354592254 "点击放大")  
图21 将20个HSP包设计成HAR包   
![](https://media:901787638354636255 "点击放大")
...
![](https://media:901787638354667256)  
图23 优化后异步执行操作（computeTaskAsync），应用冷启动耗时   
![](https://media:901787638354696257)
...
![](https://media:901787638354727258 "点击放大")
...
![](https://media:901787638354759259)  
图26 优化后异步执行操作（computeTaskAsync），应用冷启动耗时   
![](https://media:901787638354788260)
...
![](https://media:901787638354864261 "点击放大")
...
![](https://media:901787638354893262)  
图29 优化后异步执行操作（computeTaskAsync），应用冷启动耗时   
![](https://media:901787638354922263 "点击放大")
...
![](https://media:901787638355009264 "点击放大")

使用本地存储优先展示，可减少首帧展示延迟，缩短用户可见白屏时间，提升冷启动体验。  
![](https://media:901787638355041265)  
...
图31 优化前未使用本地存储数据   
![](https://media:901787638355081266 "点击放大")  
图32 优化后使用本地存储数据   
![](https://media:901787638355119267 "点击放大")
