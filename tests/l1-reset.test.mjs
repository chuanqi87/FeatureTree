...
import { createModel, treeStats, visibleTree } from "../web/model.js";

test("unexpanded L1 domains remain branches in statistics and filtering", () => {
  const model = createModel({
    roots: ["sample"],
    nodes: [{
      id: "sample", parent: null, level: "L1", children: [],
      knowledge_role: "rollup", granularity: "branch",
  ...
  assert.equal(visibleTree(model, { ...options, role: "rollup" }).rows.length, 1);
});
