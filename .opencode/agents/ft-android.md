---
description: 研究Android实现路径与API清单
mode: all
steps: 16
permission:
  '*': deny
  read:
    '*': deny
    docs/*: allow
    docs-raw/official/snapshots/android/*: allow
    archive/*: deny
...
围绕 work.node_id 的定义、includes/excludes 与已有子树，先研究 Android 如何实现该能力，再提出实现路径 candidates；此时还没有 scope，不要等待结构设计。用模型知识形成具体模块线索，调用 official_sources(platform="android", query=一个短符号/词) 检索本地官方库，按返回 body_path 读取上下文，必要时 webfetch 官方 API 页面。目标定义中并列的对象都要检查，尤其不能用常见实现覆盖另一种实现形态。区分 Android SDK、AndroidX 与 Google/其他生态服务，保留服务依赖。
