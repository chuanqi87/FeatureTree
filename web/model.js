...
import { matchingClaims } from "./quality.js";
...
export function visibleTree(
  model,
  { domain, query, collapsed, progressState = "", confidence = "", deviceReview = "", role = "" },
) {
  const terms = query.trim().toLocaleLowerCase().split(/\s+/).filter(Boolean);
  const exactId = model.byId.has(query.trim()) ? query.trim() : "";
  const filtering = terms.length > 0 || Boolean(progressState || confidence || deviceReview || role);
  const inScope = new Set(scopeIds(model, domain));
  const matches = new Set(
    [...inScope].filter(
      (id) =>
        (exactId ? id === exactId : terms.every((term) => model.searchIndex.get(id).includes(term))) &&
        (!progressState ||
          model.byId.get(id).comparison_progress.state === progressState) &&
        (!role || model.byId.get(id).knowledge_role === role) &&
        (!(confidence || deviceReview) || matchingClaims(model.byId.get(id), { confidence, deviceReview }).length > 0),
    ),
  );
  ...
  return { rows, matchedIds: [...matches], matchCount: matches.size, scopeCount: inScope.size };
}
