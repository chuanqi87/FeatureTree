...
There are multiple ways to request user location information on Android, and
the best path varies by use case. This guide describes different location access
scenarios and explains how to request location permissions
in each case.

To begin, review the best practices for
all [runtime permissions](https://developer.android.com/training/permissions/requesting) as well as the broader guidance for [minimizing
permissions requests.](https://developer.android.com/privacy-and-security/minimize-permission-requests)

## Types of location access

Each permission has a combination of the following characteristics:

* **Category**: Either [foreground location](https://developer.android.com/training/location/permissions#foreground) or [background location](https://developer.android.com/develop/sensors-and-location/location/permissions/background).
* **[Accuracy](https://developer.android.com/training/location/permissions#accuracy)**: Either precise location or approximate location.
...
* Within a navigation app, a feature allows users to get turn-by-turn
  directions.
* Within a messaging app, a feature allows users to share their current
  location with another user.
...
**Note:** Beginning with Android 12, your app can call
[`getLocationPowerSaverMode()`](https://developer.android.com/reference/android/os/PowerManager#getLocationPowerSaveMode()) to check how the device's location features
behave when Battery Saver is active. If this returns
[`LOCATION_MODE_FOREGROUND_ONLY`](https://developer.android.com/reference/android/os/PowerManager#LOCATION_MODE_FOREGROUND_ONLY), your app will continue to receive
location updates while in the foreground or running a foreground service when
Battery Saver is on, even if the screen is off.

**One-time access** In addition to accessing location data in the foreground,
some use cases have the additional distinction of only needing to access user
location at specific points in the app experience, as opposed to consistently
updating throughout app usage. For these cases, apps should use the [location
button](https://developer.android.com/guide/topics/permissions/private-alternatives/location-button) to
request location access for that individual session of app usage. Additionally,
if your app targets Android 17 (API level 37) or later and only contains
features that require session-based location access, Google Play policy requires
you to use the location button. For more details, see the [location button
policy](https://support.google.com/googleplay/android-developer/answer/16909972).
...
* Within a family location sharing app, a feature allows users to continuously
  share location with family members.
* Within an IoT app, a feature allows users to configure their home devices
  such that they turn off when the user leaves their home and turn back on
...
[foreground location](https://developer.android.com/training/location/permissions#foreground) section. The background location precision is the same
as the [foreground location precision](https://developer.android.com/training/location/permissions#accuracy), which depends on the location
...
**Note:** The Google Play Store has a [location policy](https://support.google.com/googleplay/android-developer/answer/9799150) concerning device
location, restricting background location access to apps that need it for their
core features and meet related policy requirements.
...
especially the `ACCESS_FINE_LOCATION` permission. See the [Getting to know the
