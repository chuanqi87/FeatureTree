...
- <doc://com.apple.documentation/documentation/SwiftUI/TabView> provides a set of views that people can scroll through using the Digital Crown.
- <doc://com.apple.documentation/documentation/SwiftUI/NavigationSplitView> toggles between a source list and a set of detail views.
- <doc://com.apple.documentation/documentation/SwiftUI/NavigationStack> moves into and back out of a hierarchy of views.

Use these features to make your app consistent and visually coherent:

- <doc://com.apple.documentation/documentation/SwiftUI/Toolbars> guarantee consistent size and placement for buttons.
- <doc://com.apple.documentation/documentation/SwiftUI/View/matchedGeometryEffect(id:in:properties:anchor:isSource:)> animates changes to persistent elements across different tabs.
- <doc://com.apple.documentation/documentation/SwiftUI/Color> provides opportunities for branding, glanceable information, and additional context.
- <doc://com.apple.documentation/documentation/SwiftUI/Material> help create consistency, a clear hierarchy, and a sense of place.

For design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/designing-for-watchos>.

## Scroll through views using the Digital Crown

The Digital Crown provides a precise input device that people can use to navigate between apps, scroll through an app’s content, and make precise selections without obscuring the display. To get the most out of the Digital Crown, emphasize vertical scrolling over horizontal scrolling in your app and use system controls that already support the Digital Crown, like the <doc://com.apple.documentation/documentation/SwiftUI/Picker>. In addition, watchOS 10 adds the ability to scroll vertically through a set of pages.

To create vertical pagination, use a <doc://com.apple.documentation/documentation/SwiftUI/TabView> and set the <doc://com.apple.documentation/documentation/SwiftUI/View/tabViewStyle(_:)> to <doc://com.apple.documentation/documentation/SwiftUI/TabViewStyle/verticalPage>.
...
For additional design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/digital-crown> and <doc://com.apple.documentation/design/Human-Interface-Guidelines/page-controls>.

## Toggle between a source list and detail views

In macOS, iOS, and iPadOS, split views are used to present two or more columns of data. In watchOS 10, a <doc://com.apple.documentation/documentation/SwiftUI/NavigationSplitView> is the ideal way to display a source list and the corresponding detail views. If the context is large enough to show multiple columns, the system displays a list of items in the leading column. People can then choose one or more items in a leading column to display details about those items in subsequent columns.

Similar to the experience on an iPhone in portrait orientation, the `NavigationSplitView` in watchOS 10 only shows one column at a time. For example, your app may display a <doc://com.apple.documentation/documentation/SwiftUI/List> in the source view. When someone selects an item from the list, the `NavigationSplitView` automatically animates to the corresponding detail view. When the person taps the list icon, the system returns to the source view. The system automatically handles animating the transition between source and detail view.
...
![A screenshot of an Apple Watch showing a split view's detail view. The detail view is a vertical tab view, with the third tab selected.](images/com.apple.watchOS-Apps/split-view-detail-view@2x.png)



![ A screenshot of an Apple Watch showing a split view's source view. The source view shows a list that displays a label for each item.](images/com.apple.watchOS-Apps/split-view-source-list@2x.png)

> Important: While the code for the `List` and the `TabView` look very similar, there is a significant difference. The `List` expects an <doc://com.apple.documentation/documentation/Swift/Optional> for the `selected` value. If `selected` is `nil`, that means nothing is selected. If it’s not `nil`, the system unwraps its value and compares the value to the `id` for each item in the list. On the other hand, the `TabView` doesn’t unwrap the `selected` value. It just compares `selected` to the tab item’s <doc://com.apple.documentation/documentation/SwiftUI/View/tag(_:includeOptional:)>. As a result, you either need to unwrap `selected` before passing it to the tab view, or you need to wrap the tag values in an `Optional` before passing them to the `tag(_:)` modifier.
...
For additional design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/split-views>.

## Move through a hierarchy of views

While the `TabView` and `NavigationSplitView` provide new navigation paradigms, they aren’t your only option. If your app doesn’t pivot between a detail view and a source list, or if it needs more than a few vertically paginated tabs, consider using a <doc://com.apple.documentation/documentation/SwiftUI/NavigationStack>. While the `NavigationStack` isn’t a new feature, it is still an effective way to navigate an arbitrary hierarchy of views.

To create a `NavigationStack`:

1. Create an array to store the stack.
2. Pass a <doc://com.apple.documentation/documentation/SwiftUI/Binding> to the array as the `path` to the `NavigationStack` constructor.
3. Use the <doc://com.apple.documentation/documentation/SwiftUI/View/navigationDestination(for:destination:)> modifier to provide the view for the current data from the front of the array.

Your app can then modify the array directly, or use <doc://com.apple.documentation/documentation/SwiftUI/NavigationLink> instances. These links push a value onto the array when someone taps it.
...
![A screenshot of an Apple Watch showing a navigation stack's root view. The view has a large title, a toolbar button to navigate to the leaf view, and a label.](images/com.apple.watchOS-Apps/navigation-stack-root-view@2x.png)



![A screenshot of an Apple Watch showing a navigation stack's leaf view. The view has a back button and a label.](images/com.apple.watchOS-Apps/navigation-stack-leaf-view@2x.png)
...
If your app uses a `NavigationSplitView` or `NavigationStack`, you can place buttons in a <doc://com.apple.documentation/documentation/SwiftUI/View/toolbar(content:)> and provide a consistent size, location, and appearance for the buttons. If the view has scrolling content, the buttons remain visible above the content.
...
You can also place a button in the scrolling view by using the <doc://com.apple.documentation/documentation/SwiftUI/ToolbarItemPlacement/primaryAction> placement. By default, a scrolling toolbar button remains hidden until people reveal it by scrolling up. People frequently scroll to the top of a scrolling view, so discovering a toolbar button is automatic.
...
For additional design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/toolbars>.

## Provide continuity with persistent elements

You can display the same element on multiple pages to create a sense of continuity between the pages. For example, when working with vertical tab views, use the <doc://com.apple.documentation/documentation/SwiftUI/View/matchedGeometryEffect(id:in:properties:anchor:isSource:)> modifier to animate changes to an element’s size and position between tabs.
...
Normal views can just set the <doc://com.apple.documentation/documentation/SwiftUI/View/background(alignment:content:)> to the desired color. However, for anything presented by a `NavigationSplitView`, `NavigationStack`, or `TabView`, use the <doc://com.apple.documentation/documentation/SwiftUI/View/containerBackground(_:for:)> modifier instead and pass in either the <doc://com.apple.documentation/documentation/SwiftUI/ContainerBackgroundPlacement/navigation> or <doc://com.apple.documentation/documentation/SwiftUI/ContainerBackgroundPlacement/tabView> value for the placement. The `containerBackground` modifier also supports displaying a gradient using the <doc://com.apple.documentation/documentation/SwiftUI/Color/gradient> modifier.
...
![A screenshot of a split view's source view. The source view has a green gradient background.](images/com.apple.watchOS-Apps/background-color-navigation@2x.png)



![A screenshot of an Apple Watch showing a tab view. The tab view has a blue gradient background.](images/com.apple.watchOS-Apps/background-color-tab@2x.png)

For additional design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/color>.

## Indicate hierarchy and context with materials

Use <doc://com.apple.documentation/documentation/SwiftUI/Material> effects to improve legibility, define an information hierarchy, and provide context.
...
Additionally, the system provides vibrant text labels in <doc://com.apple.documentation/documentation/SwiftUI/ShapeStyle/primary>, <doc://com.apple.documentation/documentation/SwiftUI/ShapeStyle/secondary-swift.property>, <doc://com.apple.documentation/documentation/SwiftUI/ShapeStyle/tertiary-swift.property>, and <doc://com.apple.documentation/documentation/SwiftUI/ShapeStyle/quaternary-swift.property>  prominence levels for creating a typographic hierarchy. You can also add vibrant versions of all the system colors to ensure legibility over full-color backgrounds.

For example, use the <doc://com.apple.documentation/documentation/SwiftUI/View/foregroundStyle(_:)> to set a hierarchical text style. Or use a <doc://com.apple.documentation/documentation/SwiftUI/PrimitiveButtonStyle/borderedProminent> button style to make a button more prominent.
...
For additional design guidance, see Human Interface Guidelines > <doc://com.apple.documentation/design/Human-Interface-Guidelines/materials>.
