...
Android uses intents and their associated extras to let users share information quickly
...
**Figure 1.** The Android Sharesheet.
...
* Store the data in your own `ContentProvider`, making sure that other
  ...
  access is to use [per-URI permissions](https://developer.android.com/training/permissions/restrict-interactions#uri), which are
  temporary and only grant access to the receiving application. To create a
  `ContentProvider` like this, use the
...
common MIME types when sending data in Android:
...
**Figure 2.** Text content preview on the Android Sharesheet.
...
**Figure 3.** Custom actions on the Android Sharesheet.

On Android 14 (API Level 34) and later, apps can add custom actions to the Android Sharesheet.
...
**Figure 4.** Custom targets on the Android Sharesheet.
...
is to expose relevant people or devices that your app provides.
...
**Figure 5.** The `ACTION_SEND` intent resolver.
