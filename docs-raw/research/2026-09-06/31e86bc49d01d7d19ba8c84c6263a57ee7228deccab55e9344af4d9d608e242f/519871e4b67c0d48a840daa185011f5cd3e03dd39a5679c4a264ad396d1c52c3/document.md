...
> Important: Always keep your private key secure and never share it. If your private key is compromised, you need to re-create a private key and client secret, and reupload it to your [Apple Ads](https://ads.apple.com) account.
...
- `access_token`: String. Your `access_token` is a requirement to make calls to Apple Ads Platform API endpoints. See [Managing Ad Accounts and API Access](/documentation/Apple-Ads-Platform-API/access-overview) for a complete walkthrough from this token to your first real request. Your `access_token` is valid for the number of seconds that `expires_in` specifies.
