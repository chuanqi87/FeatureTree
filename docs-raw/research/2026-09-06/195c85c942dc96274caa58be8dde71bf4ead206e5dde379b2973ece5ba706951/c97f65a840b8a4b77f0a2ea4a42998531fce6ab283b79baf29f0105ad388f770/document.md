...
You perform biometric authentication in your app using an [LAContext](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext) instance, which brokers interaction between your app and the Secure Enclave. Begin by creating a context:
...
Before attempting to authenticate, test to make sure that you actually have the ability to do so by calling the [canEvaluatePolicy(_:error:)](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext/canEvaluatePolicy(_:error:)) method:
...
Choose a value from the [LAPolicy](doc://com.apple.documentation/documentation/LocalAuthentication/LAPolicy) enumeration for which to test. The policy controls how the authentication behaves. For example, the [deviceOwnerAuthentication](doc://com.apple.documentation/documentation/LocalAuthentication/LAPolicy/deviceOwnerAuthentication) policy used in this sample indicates that reverting to a passcode is allowed when biometrics fails or is unavailable. Alternatively, you can indicate the [deviceOwnerAuthenticationWithBiometrics](doc://com.apple.documentation/documentation/LocalAuthentication/LAPolicy/deviceOwnerAuthenticationWithBiometrics) policy, which doesn’t allow reverting to the device passcode.

### Evaluate a Policy

When you’re ready to authenticate, call the [evaluatePolicy(_:localizedReason:reply:)](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext/evaluatePolicy(_:localizedReason:reply:)) method, using the same policy you already tested:
...
You test what kind of biometry the device supports by reading the context’s [biometryType](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext/biometryType) parameter:

```swift
faceIDLabel.isHidden = (state == .loggedin) || (context.biometryType != .faceID)
```

This parameter only contains a meaningful value after you run the [canEvaluatePolicy(_:error:)](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext/canEvaluatePolicy(_:error:)) method on the context at least once.
...
- You’ve previously invalidated the context with a call to the [invalidate()](doc://com.apple.documentation/documentation/LocalAuthentication/LAContext/invalidate()) method.

For a complete list of possible error conditions, see [LAError.Code](doc://com.apple.documentation/documentation/LocalAuthentication/LAError-swift.struct/Code).
