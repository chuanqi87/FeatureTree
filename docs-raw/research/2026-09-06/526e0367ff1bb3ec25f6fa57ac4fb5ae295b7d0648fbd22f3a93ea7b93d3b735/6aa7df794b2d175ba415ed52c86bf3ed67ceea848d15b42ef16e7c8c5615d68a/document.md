...
|`scope`                |String  |**(Required)** Defines the access permissions you request from the user, and limits the authorization of the access token you receive. ![](spacer) The value is `business.api` or `school.api`.|
...
|----------------------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|`access_token`        |String  |Your `access_token` is a requirement to make calls to Apple School Manager and Apple Business API endpoints. ![](spacer) Your `access_token` is valid for the number of seconds that `expires_in` specifies.|
