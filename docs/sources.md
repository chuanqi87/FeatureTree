...
资料发现入口见 [官方文档入口](official-documentation-entrypoints.md)，读取命令见 [官方资料库](official-corpus.md)。`docs-raw/` 是来源库，不是已经完成的节点知识或锁定版本的 SDK 全集。
...
来源更新后应重新核对受影响的判断。结构化记录遵循 [比较模型](comparison-model.md)，置信度和真机需求遵循 [知识置信度](knowledge-confidence.md)。
