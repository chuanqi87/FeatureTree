...
2. 创建手势收集干预处理函数，在处理函数中可调用以下接口对响应识别器和触摸识别器进行干预。
   ...
   * 通过[OH_ArkUI_GestureRecognizer_IsHostBelongsTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturerecognizer_ishostbelongsto)接口判断手势识别器是否为指定节点的后代。
   * 通过[OH_ArkUI_TouchRecognizer_IsHostBelongsTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_touchrecognizer_ishostbelongsto)接口判断触摸识别器是否为指定节点的后代。
   * 通过[OH_ArkUI_GestureCollectInterceptInfo_SetGestureCollectIntervention](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturecollectinterceptinfo_setgesturecollectintervention)接口设置收集干预策略。
