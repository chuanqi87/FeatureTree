...
![](https://media:901787638270554503)  
* 手机与电脑碰一碰需登录同一华为账号方可进行分享。
* 电脑设备在API版本26.0.0及以上可以进行精准碰一碰。  

| |手机碰手机|手机碰电脑|
|:---|:-----------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|手机发送|通过手机顶部的相碰触发，文件被设备接收，接收端将媒体文件存储到图库中，非媒体文件存储到文件管理器中。 ![](https://media:901787638270819504 "点击放大") <br />|碰一碰： 通过手机顶部与电脑窗口的相碰触发，使用约束详见手机与电脑碰一碰分享[概述](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/knock-share-pc-phones-overview)使用约束章节，文件应被窗口所属的应用接收，统一存储入沙箱文件中。 ![](https://media:901787638271155505 "点击放大") 精准碰一碰： 手机向电脑碰一碰传输文件时，系统自动识别电脑屏幕触发点坐标位置： 若触发点位置存在展示图片或视频，接收的新文件自动插入到该位置； 若触发点位置无任何图片或视频元素，接收文件默认追加到文件列表末尾。 ![](https://media:901787638271460506 "点击放大")|
|手机接收|通过手机顶部的相碰触发，文件被设备接收，接收端将媒体文件存储到图库中，非媒体文件存储到文件管理器中。 ![](https://media:901787638270819504 "点击放大") <br />|碰一碰： 通过手机顶部与电脑窗口的相碰触发，使用约束详见手机与电脑碰一碰分享[概述](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/knock-share-pc-phones-overview)使用约束章节，文件被手机设备接收，接收端将媒体文件存储到图库中，非媒体文件存储到文件管理器中。 ![](https://media:901787638271906507 "点击放大") 精准碰一碰： 电脑向手机碰一碰传输文件时，系统自动识别电脑屏幕触发点坐标位置： 若触发点位置存在展示图片或视频，则分享该文件； 若触发点位置无任何图片或视频元素，则不发送任何文件。 ![](https://media:901787638272232508 "点击放大")|

#### 实现原理

![](https://media:901787638272943509 "点击放大")
...
![](https://media:901787638273527510 "点击放大")
...
1. 分享数据构建 在分享数据时，分享发起方需要构建[SharedRecord](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/share-system-share#sharedrecord)。在文件分享的场景中，发起方在构造此参数时，必须传入uri和utd这两个属性。其中，uri属性仅在文件分享场景中为必填项，而在内容分享场景下content属性则为必填项，uri属性不需要传值。

   ![](https://media:901787638273555511)  
...
1. 设备版本校验 开发时需优先增加双重校验：判断设备是否为电脑，同时校验系统API版本≥26.0.0，不满足条件时禁用精准碰一碰逻辑，降级为普通碰一碰分享。
...
2. 触发点位置监听适配 基于电脑窗口ID监听碰一碰事件，获取碰一碰触发点屏幕坐标，关联页面图片元素位置，实现位置匹配与文件插入/分享逻辑。
   ...
   * 电脑端仍需传入[windowId](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/share-harmony-share#basecapabilityregistry)及[capabilities](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/share-harmony-share#recvcapabilityregistry)配置，接口调用规则与普通碰一碰保持一致。
...
3. 业务逻辑分支处理
   ...
   * 发送端：未手动选择文件时，使用[SharableTarget.getInfo()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/share-harmony-share#getinfo)接口获取触碰点位置，通过触发点位置匹配图片，无匹配图片则直接终止分享流程。
