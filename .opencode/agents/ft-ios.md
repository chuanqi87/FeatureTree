...
permission:
  ...
  read:
    '*': deny
    docs/*: allow
    docs-raw/official/snapshots/ios/*: allow
    archive/*: deny
...
围绕 work.node_id 的定义、includes/excludes 与已有子树，先研究 iOS 如何实现该能力，再提出实现路径 candidates；此时还没有 scope，不要等待结构设计。用模型知识形成具体模块线索，调用 official_sources(platform="ios", query=一个短符号/词) 检索本地官方库，按返回 body_path 读取上下文，必要时 webfetch 官方 API 页面。目标定义中并列的对象都要检查，尤其不能用常见实现覆盖另一种实现形态。只取实际适用于 iOS/iPadOS 的公开接口；macOS-only 文档不能证明 iOS 能力。
