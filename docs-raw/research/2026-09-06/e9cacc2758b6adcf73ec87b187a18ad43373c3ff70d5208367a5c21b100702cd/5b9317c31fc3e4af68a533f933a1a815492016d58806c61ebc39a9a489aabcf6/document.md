...
`java.text`. An app that requires internationalization functionalities beyond
...
* The APIs in the Android framework don't replace Android’s support for
  [localizing with
...
* In some cases, the Android framework supports more characters than
  the ICU libraries do. This is true, for example, of the `android.text` class's
  support for emoji.
...
ICU on Android doesn't observe the user's 24h/12h time format setting, obtained from
[`DateFormat.is24HourFormat()`](https://developer.android.com/reference/android/text/format/DateFormat#is24HourFormat(android.content.Context)).
To observe this setting, either use
[`DateFormat`](https://developer.android.com/reference/android/text/format/DateFormat) or
...
For example, this code generates a string with the current time that observes the user's 12h/24h setting:
...
[`Transliterator`](https://developer.android.com/reference/android/icu/text/Transliterator)
is provided to transliterate text from one format to another. The set of available transliteation
IDs is unstable across Android releases and devices. Device manufacturers might add extra
transliteration IDs. Developers must check the available IDs, obtained from
[`Transliterator.getAvailableIDs()`](https://developer.android.com/reference/android/icu/text/Transliterator#getAvailableIDs()),
