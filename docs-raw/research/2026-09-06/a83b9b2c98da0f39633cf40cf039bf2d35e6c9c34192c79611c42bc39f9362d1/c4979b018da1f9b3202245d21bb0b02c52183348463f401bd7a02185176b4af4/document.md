...
| `raw/` | Arbitrary files to save in their raw form. To open these resources with a raw `InputStream`, call `Resources.openRawResource` with the resource ID, which is `R.raw.filename`.  However, if you need access to the original filenames and file hierarchy, consider saving resources in the `assets/` directory instead of `res/raw/`. Files in `assets/` aren't given a resource ID, so you can only read them using `AssetManager`. |
...
2. Save the appropriate alternative resources in this new directory. The resource files must be
...
resources](https://developer.android.com/guide/topics/resources/available-resources#Compatibility).
...
* You can specify multiple qualifiers for a single set of resources, separated by dashes. For
  example, `drawable-en-rUS-night` applies to US-English devices in night mode.
* The qualifiers must be in the order listed in [Table 2](https://developer.android.com/guide/topics/resources/available-resources#table2).
...
* Only one value for each qualifier type is supported. For example, if you want to use
  ...
  `drawable-es-fr/`. Instead, you need two resource directories, such as
  `drawable-es/` and `drawable-fr/`, which contain the appropriate files.
...
best-matching resource](https://developer.android.com/guide/topics/resources/available-resources#BestMatch).
...
* The *resource type*: each resource is grouped into a "type," such as `string`
  or `drawable`.
* The *resource name*, which is the filename
...
* *`<resource_name>`* is either the resource filename
  without the extension or the `android:name` attribute value in the XML element, for
  simple values.
...
For example, if your app supports several languages, always include a `values/` directory (in which your strings are saved) *without* a [language and region qualifier](https://developer.android.com/guide/topics/resources/available-resources#LocaleQualifier). If you instead put all your string files
...
resources with the [screen density](https://developer.android.com/guide/topics/resources/available-resources#DensityQualifier) qualifier. Even without default
...
![](/static/images/resources/res-selection-flowchart.png)
...
2. Find the next-highest-precedence qualifier in the list ([Table 2](https://developer.android.com/guide/topics/resources/available-resources#table2)).
   (Start with MCC.)
...
4. Eliminate resource directories that don't include this qualifier. In this example, the system
...
5. Repeat steps two, three, and four until only one directory remains. In this example, night
...
**Note:** The *precedence* of the qualifier (in [Table 2](https://developer.android.com/guide/topics/resources/available-resources#table2)) is more important
