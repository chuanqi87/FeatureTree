...
import { safeUrl } from "../web/format.js";
import { buildKnowledgeIndex, searchKnowledge } from "../web/search.js";
import { matchingClaims, reviewSelection } from "../web/quality.js";
...
test("quality filters require both conditions on the same claim", () => {
  const node = { quality_claims: [
    { confidence: "high", device_requirement: "not_required", device_state: "not_required" },
    { confidence: "low", device_requirement: "required", device_state: "pending" },
  ] };
  assert.equal(matchingClaims(node, { confidence: "high", deviceReview: "required" }).length, 0);
  assert.equal(matchingClaims(node, { confidence: "low", deviceReview: "required" }).length, 1);
...
test("unassessed fixtures are never treated as high confidence or no device review", () => {
  ...
  fixture.parent = null;
  fixture.children = [];
  fixture.quality_claims = [{ confidence: "unassessed", device_requirement: "unassessed", device_state: "unassessed" }];
  fixture.knowledge = { programming_model: "LEGACYPROSE", comparisons: [] };
  const ungraded = createModel({ ...snapshot, roots: [fixture.id], nodes: [fixture] });
  assert.equal(visibleTree(ungraded, { ...options, confidence: "high" }).matchCount, 0);
  assert.equal(visibleTree(ungraded, { ...options, deviceReview: "not_required" }).matchCount, 0);
  assert.equal(visibleTree(ungraded, { ...options, confidence: "unassessed" }).matchCount, 1);
  const snippets = searchKnowledge(buildKnowledgeIndex(ungraded), "LEGACYPROSE")[0].snippets;
