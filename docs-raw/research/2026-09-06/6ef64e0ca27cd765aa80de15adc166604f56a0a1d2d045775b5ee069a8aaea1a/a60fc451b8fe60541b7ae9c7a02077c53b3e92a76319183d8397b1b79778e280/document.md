...
The sample centralizes all MetricKit integration in a `MetricsCollector` class. This class initializes a [`MetricManager`](/documentation/MetricKit/MetricManager) with the state reporting domains the app uses — one for experiments and one for user activity:
...
The sample listens for two types of reports using async sequences on [`MetricManager`](/documentation/MetricKit/MetricManager). Metric reports contain aggregated performance data, and diagnostic reports contain information about crashes, hangs, and other issues.
...
The sample defines a `@ReportableMetadata` macro to describe the experiment context, and a [`StateReportingDomain`](/documentation/MetricKit/StateReportingDomain) to group experiment data:
...
The sample defines a `@ReportableMetadata` type and a [`StateReportingDomain`](/documentation/MetricKit/StateReportingDomain) for user activity, with constants for each tracked mode:
...
The sample uses [`SignpostIntervalMetric`](/documentation/MetricKit/SignpostIntervalMetric) to measure resource usage during the receipt scanning workflow. By wrapping the document camera session in a signpost interval, MetricKit aggregates CPU time, memory, logical writes, and duration across scanning sessions and includes them in metric reports.

The `MetricsCollector` obtains a log handle from [`MetricManager`](/documentation/MetricKit/MetricManager) and provides methods to begin and end the signpost interval:
...
The log handle that [`logHandle(category:)`](/documentation/MetricKit/MetricManager/logHandle(category:)) returns is persistent. So, MetricKit collects and aggregates all the signposts that this log handle emits. The signpost name and category identify this interval in the resulting [`SignpostIntervalMetric`](/documentation/MetricKit/SignpostIntervalMetric) objects. You can also use <doc://com.apple.documentation/documentation/os/OSSignposter> with this handle, but only [`mxSignpost(_:dso:log:name:signpostID:_:_:)`](/documentation/MetricKit/mxSignpost(_:dso:log:name:signpostID:_:_:)) populates the [`SignpostIntervalMetric`](/documentation/MetricKit/SignpostIntervalMetric) measurement properties such as CPU time, memory usage, and logical writes.
...
The resulting metric reports include an [`SignpostIntervalMetric`](/documentation/MetricKit/SignpostIntervalMetric) entry containing cumulative CPU time, peak memory, cumulative logical writes, and a histogram of interval durations.
...
Each [`DiagnosticReport`](/documentation/MetricKit/DiagnosticReport) contains a `result` property of type [`DiagnosticResult`](/documentation/MetricKit/DiagnosticResult), which is an enum indicating the kind of diagnostic:
...
The [`CrashDiagnostic`](/documentation/MetricKit/CrashDiagnostic) type provides details about the crash, including the termination reason, Mach exception type, Unix signal, and the call stack tree at the time of the crash.
