...
import LiveActivity, { ActivityOverview, ActivityBadge } from "./LiveActivity.jsx";
export default function WorkflowPage({ releaseId }) {
  ...
  useEffect(() => { if (!selected && runs.data?.items?.length) setSelected([...runs.data.items].sort((a, b) => b.updated_at.localeCompare(a.updated_at))[0].run_id); }, [selected, runs.data]);
  ...
  return <><PageHeader title="具名 Agent 工作流" description="建树链与知识链各司其职，每个阶段都有输入、交付件、问题和返工历史。"><Button type="primary" onClick={() => setCreate(true)}>创建校准工作单</Button></PageHeader>
    ...
    <ResourceState resource={detail}>{detail.data && <><PageHeader title={detail.data.plan.pipeline === "taxonomy" ? "建树分析链" : "叶子知识链"} description={selected}><Button loading={busy} onClick={() => act("start")}>启动 / 继续</Button><Button danger onClick={() => act("cancel")}>取消运行</Button></PageHeader>
      <Status value={detail.data.state.status} /><ActivityOverview activity={detail.data.activity} model={detail.data.plan.model} /><PipelineStages plan={detail.data.plan} tasks={tasks} renderTask={task => <Card key={task.id} size="small" title={detail.data.plan.stage_configs[task.stage_id].definition.agent_name || `代码：${task.stage_id}`} extra={detail.data.activity ? <ActivityBadge activity={detail.data.activity.stages[task.id]} /> : <Status value={task.status} />}>
        <Space orientation="vertical"><Typography.Text type="secondary">{task.reuse_ref ? "已复用兼容交付 · " : ""}{task.work_id} · 修订 {task.revision} · {task.attempts.length} 次尝试</Typography.Text><BatchProgress progress={detail.data.activity?.stages[task.id]?.total ? { ...detail.data.activity.stages[task.id].progress, completed: detail.data.activity.stages[task.id].completed, status: detail.data.activity.stages[task.id].status, current: detail.data.activity.stages[task.id].live?.batch } : detail.data.batch_progress?.[task.id]} />{task.outcome && <Status value={task.outcome} />}<Space><Button size="small" onClick={() => setStage(task.id)}>查看阶段</Button>{task.reuse_ref && <Button size="small" onClick={() => setArtifact(task.reuse_ref)}>复用依据</Button>}{task.result_ref && <Button size="small" onClick={() => setArtifact(task.result_ref)}>交付件</Button>}</Space></Space>
      </Card>} /><JsonDetails value={detail.data.plan.works} title="固定的范围、完整输入 ID 与上游版本" /></>}</ResourceState>
    ...
    <Drawer open={!!stage} title={selectedStage?.stage_id} onClose={() => setStage(null)} size="large">{selectedStage && <><Space wrap><Button onClick={() => act("retry", stage)} disabled={selectedStage.status !== "failed" || detail.data.activity?.stages[stage]?.status === "running"}>重试执行</Button><Button onClick={() => act("revalidate", stage)} disabled={selectedStage.status !== "failed" || detail.data.activity?.stages[stage]?.status === "running"}>复核保留答案</Button><Button onClick={() => act("revise", stage)}>定向修订</Button><Button onClick={() => act("supplement", stage, selectedStage.work_id)}>补充已提出的来源</Button></Space><LiveActivity activity={detail.data.activity?.stages[selectedStage.id]} onArtifact={setArtifact} /><JsonDetails value={detail.data.plan.stage_configs[selectedStage.stage_id].definition} title="Agent 职责、依赖与契约" /><JsonDetails value={selectedStage.attempts} title="每次尝试、实际模型用量与错误" /><JsonDetails value={selectedStage.history} title="被替代的交付件版本" /><JsonDetails value={selectedStage.feedback} title="返工问题" /></>}</Drawer>
