...
You get game controller input from the device’s input profile. The [`GCDevicePhysicalInput`](/documentation/GameController/GCDevicePhysicalInput) protocol declares the common properties for the input profile. The [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) and [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) classes provide the concrete implementations for gamepads and arcade sticks, and racing wheels, respectively.

For gamepads and arcade sticks, first get the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) object from the [`GCController`](/documentation/GameController/GCController) object using the [`input`](/documentation/GameController/GCController/input) property.
...
For racing wheels, first get the [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) object from the [`GCRacingWheel`](/documentation/GameController/GCRacingWheel) object using the [`wheelInput`](/documentation/GameController/GCRacingWheel/wheelInput) property.
...
The Game Controller framework processes input events, and invokes any configured callbacks, on the [`GCController`](/documentation/GameController/GCController) or [`GCRacingWheel`](/documentation/GameController/GCRacingWheel) [`handlerQueue`](/documentation/GameController/GCDevice/handlerQueue) property of the device. The [`handlerQueue`](/documentation/GameController/GCDevice/handlerQueue) property is the main queue by default. To configure a dedicated queue for input processing, set the [`queue`](/documentation/GameController/GCDevicePhysicalInput/queue) property of the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) or [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) object.
...
To get objects representing specific elements of a controller, use the subscript notation with the collection properties from an object conforming to the [`GCDevicePhysicalInputState`](/documentation/GameController/GCDevicePhysicalInputState) protocol. This includes the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) and [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) objects.
...
Both the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) and [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) objects track the last received input state from the game controller. If you poll for input, but just need the current input values, get the elements from the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) and [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) objects and read their input values.
...
Reading a single input value is an atomic operation. However, the current input state can change between reading distinct input values. To avoid these inconsistencies, use the [`GCDevicePhysicalInput`](/documentation/GameController/GCDevicePhysicalInput). [`capture()`](/documentation/GameController/GCDevicePhysicalInput/capture()) method to obtain a snapshot of the input state at the current moment. Then read the input values from the snapshot. Snapshots carry very little performance overhead as long as your game doesn’t hold onto the snapshot for long periods of time.
...
In your game loop, handle all the inputs that occur after the last iteration of your game loop using the [`nextInputState()`](/documentation/GameController/GCDevicePhysicalInput/nextInputState()) method. Call the [`nextInputState()`](/documentation/GameController/GCDevicePhysicalInput/nextInputState()) method until the queue is empty.

Optionally, assign a callback to the [`inputStateAvailableHandler`](/documentation/GameController/GCDevicePhysicalInput/inputStateAvailableHandler) property of the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) or [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) objects. The Game Controller framework calls this block when a new input state becomes available, and not again until your game processes all input states in the queue.
...
Each call to the [`nextInputState()`](/documentation/GameController/GCDevicePhysicalInput/nextInputState()) method returns a snapshot of the oldest input state in the queue. Use the snapshot to access individual elements and read their input values.
...
To determine whether an element’s input changed compared to the previous input state, use the [`change(for:)`](/documentation/GameController/GCDevicePhysicalInputStateDiff/change(for:)) method. To get a collection of all the elements with changed inputs compared to the previous input state, use the [`changedElements()`](/documentation/GameController/GCDevicePhysicalInputStateDiff/changedElements()-2zzwm) method.
...
Handle when the queue overflows. If the queue is empty, the [`nextInputState()`](/documentation/GameController/GCDevicePhysicalInput/nextInputState()) method returns `nil`. If the queue overflows, the [`changedElements()`](/documentation/GameController/GCDevicePhysicalInputStateDiff/changedElements()-2zzwm) method returns `nil` and the [`change(for:)`](/documentation/GameController/GCDevicePhysicalInputStateDiff/change(for:)) method returns [`GCDevicePhysicalInputElementChange.unknownChange`](/documentation/GameController/GCDevicePhysicalInputElementChange/unknownChange).
...
For example, a button element has a pressed input representing its pressed state. Assign a block to the [`pressedDidChangeHandler`](/documentation/GameController/GCPressedStateInput/pressedDidChangeHandler) property of the button’s [`pressedInput`](/documentation/GameController/GCButtonElement/pressedInput) property to take an action when the user presses it.
...
To receive a callback when the inputs of any element change, assign a callback to the [`elementValueDidChangeHandler`](/documentation/GameController/GCDevicePhysicalInput/elementValueDidChangeHandler) property of the [`GCControllerLiveInput`](/documentation/GameController/GCControllerLiveInput) or [`GCRacingWheelInput`](/documentation/GameController/GCRacingWheelInput) objects. When input occurs, the Game Controller framework calls the block once for each element that changes. Your code reads the latest values from the element that Game Controller passes to the handler.
