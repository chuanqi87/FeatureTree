...
import { confidenceOptions, deviceReviewOptions } from "../quality.js";
    ...
    <Card className="filter-card" size="small">
      <Form layout="inline" className="filter-form">
        ...
        <Form.Item label="知识置信度">
          <Select aria-label="知识置信度" value={explorer.confidence} onChange={explorer.setConfidence} options={confidenceOptions} />
        </Form.Item>
        <Form.Item label="真机复核">
          <Select aria-label="真机复核" value={explorer.deviceReview} onChange={explorer.setDeviceReview} options={deviceReviewOptions} style={{ width: 210 }} />
        </Form.Item>
        <Form.Item label="节点类型">
          <Select aria-label="节点类型" value={explorer.role} onChange={explorer.setRole} options={[
            { value: "", label: "全部类型" }, { value: "leaf", label: "仅叶子节点" }, { value: "rollup", label: "仅分支节点" },
          ]} />
        </Form.Item>
        <Form.Item label="导出预算（知识项）">
          <InputNumber aria-label="导出预算" min={1} precision={0} value={explorer.reviewLimit} onChange={explorer.setReviewLimit} placeholder="不限" />
        </Form.Item>
        <Form.Item>
      ...
      <Typography.Paragraph type="secondary" style={{ margin: "12px 0 0" }}>
        两项筛选作用于同一条结论。高可信不等于已实测；未评估不等于无需实测。优先级：实测不符 → 必须复核 → 建议抽样 → 补证据/评级。
