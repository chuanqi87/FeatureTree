...
import { reviewSelection } from "../quality.js";
...
export function useExplorer() {
  ...
  const filtering = Boolean(query.trim() || progressState || confidence || deviceReview || role);
  const tree = useMemo(
    () =>
      model
        ? visibleTree(model, { domain, query, collapsed, progressState, confidence, deviceReview, role })
        : { rows: [], matchedIds: [], matchCount: 0, scopeCount: 0 },
    [model, domain, query, collapsed, progressState, confidence, deviceReview, role],
  );
  const data = useMemo(
    () =>
      model
  ...
  const reviewScope = useMemo(
    () => model ? reviewSelection(model, tree.matchedIds, { domain, query, progressState, confidence, deviceReview, role }, reviewLimit) : null,
    [model, tree, domain, query, progressState, confidence, deviceReview, role, reviewLimit],
  );
