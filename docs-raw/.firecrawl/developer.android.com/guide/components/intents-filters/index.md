[Skip to main content](https://developer.android.com/guide/components/intents-filters#main-content)
...
- [English](https://developer.android.com/guide/components/intents-filters)
- [Deutsch](https://developer.android.com/guide/components/intents-filters?hl=de)
- [Español – América Latina](https://developer.android.com/guide/components/intents-filters?hl=es-419)
- [Français](https://developer.android.com/guide/components/intents-filters?hl=fr)
- [Indonesia](https://developer.android.com/guide/components/intents-filters?hl=id)
- [Italiano](https://developer.android.com/guide/components/intents-filters?hl=it)
- [Polski](https://developer.android.com/guide/components/intents-filters?hl=pl)
- [Português – Brasil](https://developer.android.com/guide/components/intents-filters?hl=pt-br)
- [Tiếng Việt](https://developer.android.com/guide/components/intents-filters?hl=vi)
- [Türkçe](https://developer.android.com/guide/components/intents-filters?hl=tr)
- [Русский](https://developer.android.com/guide/components/intents-filters?hl=ru)
- [עברית](https://developer.android.com/guide/components/intents-filters?hl=he)
- [العربيّة](https://developer.android.com/guide/components/intents-filters?hl=ar)
- [فارسی](https://developer.android.com/guide/components/intents-filters?hl=fa)
- [हिंदी](https://developer.android.com/guide/components/intents-filters?hl=hi)
- [বাংলা](https://developer.android.com/guide/components/intents-filters?hl=bn)
- [ภาษาไทย](https://developer.android.com/guide/components/intents-filters?hl=th)
- [中文 – 简体](https://developer.android.com/guide/components/intents-filters?hl=zh-cn)
- [中文 – 繁體](https://developer.android.com/guide/components/intents-filters?hl=zh-tw)
- [日本語](https://developer.android.com/guide/components/intents-filters?hl=ja)
- [한국어](https://developer.android.com/guide/components/intents-filters?hl=ko)

[Android Studio](https://developer.android.com/studio)

[Sign in](https://developer.android.com/_d/signin?continue=https%3A%2F%2Fdeveloper.android.com%2Fguide%2Fcomponents%2Fintents-filters&prompt=select_account)
...
- [Building an intent](https://developer.android.com/guide/components/intents-filters#Building)
  ...
  - [Forcing an app chooser](https://developer.android.com/guide/components/intents-filters#ForceChooser)
  - [Detect unsafe intent launches](https://developer.android.com/guide/components/intents-filters#DetectUnsafeIntentLaunches)
- [Receiving an implicit intent](https://developer.android.com/guide/components/intents-filters#Receiving)
  - [Example filters](https://developer.android.com/guide/components/intents-filters#ExampleFilters)
- [Match intents to other apps' intent filters](https://developer.android.com/guide/components/intents-filters#match-intent-filter)
- [Using a pending intent](https://developer.android.com/guide/components/intents-filters#PendingIntent)
  - [Specify mutability](https://developer.android.com/guide/components/intents-filters#DeclareMutabilityPendingIntent)
  - [Use explicit intents within pending intents](https://developer.android.com/guide/components/intents-filters#UseExplicitWithinPendingIntent)
- [Intent resolution](https://developer.android.com/guide/components/intents-filters#Resolution)
  - [Action test](https://developer.android.com/guide/components/intents-filters#ActionTest)
  - [Category test](https://developer.android.com/guide/components/intents-filters#CategoryTest)
  - [Data test](https://developer.android.com/guide/components/intents-filters#DataTest)
  - [Intent matching](https://developer.android.com/guide/components/intents-filters#imatch)
...
# Intents and intent filters    Stay organized with collections      Save and categorize content based on your preferences.
...
from another component by passing an `Intent` to `bindService()`. For more information, see the [Services](https://developer.android.com/guide/components/services) guide.
...
delivered through the system to start another activity: **\[1\]** _Activity A_ creates an
...
starts the matching activity ( _Activity B_) by invoking its `onCreate()` method and passing it the `Intent`.

When you use an implicit intent, the Android system finds the appropriate component to start
by comparing the contents of the intent to the _intent filters_ declared in the [manifest file](https://developer.android.com/guide/topics/manifest/manifest-intro) of other apps on the
...
Likewise, if you do _not_ declare any intent filters for an activity, then it can be started
...
_explicit_, meaning that the intent should be delivered only to the app component
defined by the component name. Without a component name, the intent is _implicit_ and the
...
_always specify the component name_. Otherwise, you cannot be certain what service
...
**Action**A string that specifies the generic action to perform (such as _view_ or _pick_).
...
`ACTION_VIEW`Use this action in an intent with `startActivity()` when you have some information that
an activity can show to the user, such as a photo to view in a gallery app, or an address to
view in a map app.`ACTION_SEND`Also known as the _share_ intent, you should use this in an intent with `startActivity()` when you have some data that the user can
...
[Kotlin](https://developer.android.com/guide/components/intents-filters#kotlin)[Java](https://developer.android.com/guide/components/intents-filters#java)More
...
**Data**The URI (a `Uri` object) that references the data to
...
`CATEGORY_BROWSABLE`The target activity allows itself to be started by a web browser to display data
referenced by a link, such as an image or an e-mail message.
`CATEGORY_LAUNCHER`The activity is the initial activity of a task and is listed in
...
`ACTION_SEND`, you can specify the _to_ recipient with the
`EXTRA_EMAIL` key, and specify the _subject_ with the
`EXTRA_SUBJECT` key.
...
**Flags**Flags are defined in the `Intent` class that function as metadata for the
...
perform an unsafe launch of a _nested intent_, which is an intent that is passed
...
component](https://developer.android.com/guide/components/fundamentals#Components) using that nested intent,
...
1. Your app creates an intent that invokes an activity in another app. Within
that intent, you add a `PendingIntent` object as an extra. This pending intent
invokes a component in your app; this component isn't exported.
2. Upon receiving your app's intent, the other app extracts the nested
`PendingIntent` object.
3. The other app invokes the `send()` method on the `PendingIntent` object.
4. After passing control back to your app, the system invokes the pending
intent using your app's context.
...
[`<action>`](https://developer.android.com/guide/topics/manifest/action-element)Declares the intent action accepted, in the `name` attribute. The value
must be the literal string value of an action, not the class constant.[`<data>`](https://developer.android.com/guide/topics/manifest/data-element)Declares the type of data accepted, using one or more attributes that specify various
aspects of the data URI (`scheme`, `host`, `port`,
`path`) and MIME type.[`<category>`](https://developer.android.com/guide/topics/manifest/category-element)Declares the intent category accepted, in the `name` attribute. The value
...
If it's important that _only your own app_ is able to start one of your components,
...
icon should be placed in the system's app launcher. If the [`<activity>`](https://developer.android.com/guide/topics/manifest/activity-element) element
does not specify an icon with `icon`, then the system uses the icon from the [`<application>`](https://developer.android.com/guide/topics/manifest/application-element)
element.
...
"system UID" (uid=1000). System apps include `system_server` and apps that set
`android:sharedUserId` to `android.uid.system`.
...
- Declaring an intent to be executed when the user performs an action with your [Notification](https://developer.android.com/develop/ui/views/notifications)
...
Unless your app is _receiving_ pending intents from other apps,
...
notifications](https://developer.android.com/training/notify-user/build-notification#reply-action). The
direct reply requires a change to the clip data in the PendingIntent object
that's associated with the reply. Usually, you request this change by passing
`FILL_IN_CLIP_DATA` as a flag to the
[`fillIn()`](https://developer.android.com/reference/android/content/Intent#fillIn(android.content.Intent,%20int))
method.
- Associating notifications with the Android Auto framework, using instances of
[`CarAppExtender`](https://developer.android.com/reference/androidx/car/app/notification/CarAppExtender).
- Placing conversations in [bubbles](https://developer.android.com/guide/topics/ui/bubbles) using instances
of `PendingIntent`. A mutable `PendingIntent` object allows the system to apply
the correct flags, such as
...
[`requestLocationUpdates()`](https://developer.android.com/reference/android/location/LocationManager#requestLocationUpdates(java.lang.String,%20long,%20float,%20android.app.PendingIntent))
...
- Scheduling alarms using [`AlarmManager`](https://developer.android.com/reference/android/app/AlarmManager).
The mutable `PendingIntent` object allows the system to add the
[`EXTRA_ALARM_COUNT`](https://developer.android.com/reference/android/content/Intent#EXTRA_ALARM_COUNT)
intent extra. This extra represents the number of times that a repeating alarm
has been triggered. By containing this extra, the intent can accurately notify
an app as to whether a repeating alarm was triggered multiple times, such as
...
1. Check that the action, package, and component fields of the base intent
    are set.
...
1. An intent that contains neither a URI nor a MIME type passes the
test only if the filter does not specify any URIs or MIME types.
2. An intent that contains a URI but no MIME type (neither explicit nor inferable from the
URI) passes the test only if its URI matches the filter's URI format
and the filter likewise does not specify a MIME type.
3. An intent that contains a MIME type but not a URI passes the test
...
its filter lists _only_ a MIME type.
