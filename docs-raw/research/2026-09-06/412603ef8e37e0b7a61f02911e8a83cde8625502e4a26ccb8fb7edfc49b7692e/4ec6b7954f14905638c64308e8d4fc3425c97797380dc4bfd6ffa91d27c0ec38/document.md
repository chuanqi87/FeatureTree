...
![](https://media:201788199364070198)  
...
|targetBundleNames^9+^|Array\<string\>|是|否|关注的目标Bundle名称。|
...
|needHide^12+^|boolean|是|否|辅助应用是否在已安装的扩展服务列表中被隐藏，true表示隐藏服务，false表示显示服务。|
|label^12+^|string|是|否|辅助应用在扩展服务列表中的名称。|
...
![](https://media:201788199364097199)  
...
![](https://media:201788199364129200)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
![](https://media:201788199364153201)  
从API version 8开始支持，从API version 12开始废弃。
...
![](https://media:201788199364180202)  
从API version 8开始支持，从API version 12开始废弃。
...
|elementId^12+^|number|否|是|组件elementId，默认值为0。|
|textAnnouncedForAccessibility^12+^|string|否|是|主动播报的内容。当应用需要主动播报时，根据实际场景设置播报内容，无特殊限制，默认值为空。|
|textResourceAnnouncedForAccessibility^18+^|Resource|否|是|主动播报的内容支持传入Resource类型，且Resource只能引用string类型资源（如$r('app.string.xxx')）。|
|customId^12+^|string|否|是|主动聚焦的组件ID，当应用需要主动聚焦时根据实际场景设置，默认值为空。|
...
![](https://media:201788199364205203)  
...
![](https://media:201788199364229204)  
...
#### accessibility.getAccessibilityExtensionList^9+^
...
#### accessibility.getAccessibilityExtensionListSync^12+^
...
获取无障碍字幕配置管理实例。  
![](https://media:201788199364257205)  
从API version 8开始支持，从API version 12开始废弃。
...
![](https://media:201788199364281206)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
如需获取系统内辅助应用信息，推荐使用[accessibility.getAccessibilityExtensionListSync](#accessibilitygetaccessibilityextensionlistsync12)。  
![](https://media:201788199364309207)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
![](https://media:201788199364341208)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
![](https://media:201788199364370209)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
![](https://media:201788199364478213)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
![](https://media:201788199364503214)  
* 注册监听的callback参数应使用具名函数而非匿名函数，否则每次调用时会创建一个新的底层对象，引起内存泄漏问题。
...
#### accessibility.offAnimationReduceStateChange^23+^
...
#### accessibility.offFlashReminderStateChange^23+^
...
#### accessibility.offAudioMonoStateChange^23+^
...
![](https://media:201788199364535215)  
...
![](https://media:201788199364565216)  
从API version 7开始支持，从API version 10开始废弃，建议使用[accessibility.isOpenAccessibilitySync](#accessibilityisopenaccessibilitysync10)替代。
...
#### accessibility.isOpenAccessibilitySync^10+^
...
![](https://media:201788199364586217)  
...
![](https://media:201788199364616218)  
从API version 7开始支持，从API version 10开始废弃，建议使用[accessibility.isOpenTouchGuideSync](#accessibilityisopentouchguidesync10)替代。
...
#### accessibility.isOpenTouchGuideSync^10+^
...
#### accessibility.isScreenReaderOpenSync^18+^
...
#### accessibility.isAnimationReduceEnabled^23+^
...
#### accessibility.isAnimationReduceEnabledSync^23+^
...
#### accessibility.isFlashReminderEnabled^23+^
...
#### accessibility.isFlashReminderEnabledSync^23+^
...
#### accessibility.isAudioMonoEnabled^23+^
...
#### accessibility.isAudioMonoEnabledSync^23+^
...
![](https://media:201788199364640219)  
...
![](https://media:201788199364662220)  
...
#### accessibility.sendAccessibilityEvent^9+^
...
#### accessibility.getTouchModeSync^20+^
