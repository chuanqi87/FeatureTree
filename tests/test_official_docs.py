...
from featuretree.corpus.cli import parser, run
...
class CorpusCommandTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root_patch=patch('featuretree.corpus.cli.ROOT',self.root)
        self.root_patch.start()
        self.addCleanup(self.root_patch.stop)

    def test_audit_writes_nested_reports_and_resolves_guide_link(self):
        with patch('featuretree.corpus.cli.emit'):
    ...
    def test_context_command_writes_candidates_under_output(self):
        ...
        with patch('featuretree.corpus.cli.Repository',return_value=self.repo), patch('featuretree.corpus.cli.emit'):
