...
当前优先级：**由协调者直接设计并写入完整三端特性树，再核实 API 锚点，最后并行深化节点知识**。设计契约见 [docs/tree-design.md](docs/tree-design.md)。历史 inventory-first 运行数据已归档至 `archive/2026-09-06-inventory-first/`，不作为当前真相源。

平台无关约束节点的定义与分类方式。Android、iOS、HarmonyOS 的信息附着在每个节点上；单平台独有能力也进入公共树。

## 当前结果

- 历史种子树有 212 个比较节点，并非三端能力完整目录；父节点和叶子都能记录平台支持与差异。
