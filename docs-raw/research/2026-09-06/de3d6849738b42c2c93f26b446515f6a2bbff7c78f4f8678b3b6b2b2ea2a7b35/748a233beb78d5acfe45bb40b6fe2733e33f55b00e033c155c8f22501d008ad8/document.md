...
For example, this sample app defines a rectangular area enclosing San Francisco as an array of [`CLLocationCoordinate2D`](doc://com.apple.documentation/documentation/CoreLocation/CLLocationCoordinate2D) coordinates.
...
The app creates the overlay objects by providing the coordinate data to an object that conforms to the [`MKOverlay`](/documentation/MapKit/MKOverlay) protocol.
...
Because MapKit defines overlays using a protocol, any class in an app can be an overlay object by conforming to the `MKOverlay` protocol, or by subclassing [`MKShape`](/documentation/MapKit/MKShape) or [`MKMultiPoint`](/documentation/MapKit/MKMultiPoint). For example, `PeakGroundAccelerationGrid` in this sample app is a custom data class representing an overlay, so it subclasses `MKShape`.

An overlay contains two key properties, a [`coordinate`](/documentation/MapKit/MKOverlay/coordinate) that defines the center point of the overlay, and the [`boundingMapRect`](/documentation/MapKit/MKOverlay/boundingMapRect) that the system expresses as an [`MKMapRect`](/documentation/MapKit/MKMapRect) that completely encompasses the overlay’s content.
...
[GeoJSON](https://tools.ietf.org/html/rfc7946) is a standards-based data format for representing geographic data, and apps often receive overlay data from a server in GeoJSON format.
Rather than connect to a server, this app uses a local GeoJSON file containing `MultiPolygon` features into an `MKMultiPolygon` by using [`MKGeoJSONDecoder`](/documentation/MapKit/MKGeoJSONDecoder).
...
To specify whether an overlay is above or below content that the map provides, such as roads and labels, the app calls [`addOverlay(_:level:)`](/documentation/MapKit/MKMapView/addOverlay(_:level:)) with the `level` parameter as a value that [`MKOverlayLevel`](/documentation/MapKit/MKOverlayLevel) provides.
...
A second object, called an *overlay renderer*, handles the drawing responsibilities for displaying the overlay on the map view.
After adding an overlay, the map view calls [`mapView(_:rendererFor:)`](/documentation/MapKit/MKMapViewDelegate/mapView(_:rendererFor:)) on its delegate to create an appropriate renderer object.
...
The [`overlays`](/documentation/MapKit/MKMapView/overlays) property of [`MKMapView`](/documentation/MapKit/MKMapView) stores the registered overlays, but the order of the array doesn’t necessarily reflect their visual order on the map.
To understand the rendering order of overlays at a specific level, see [`overlays(in:)`](/documentation/MapKit/MKMapView/overlays(in:)).

### Use the standard overlay objects for common shapes

The app highlights specific map regions with basic shapes by using the standard overlay classes, including [`MKCircle`](/documentation/MapKit/MKCircle), [`MKPolyline`](/documentation/MapKit/MKPolyline), and [`MKPolygon`](/documentation/MapKit/MKPolygon).
...
The standard overlay classes define the basic shape of the overlay, and the app uses them in conjunction with the [`MKCircleRenderer`](/documentation/MapKit/MKCircleRenderer), [`MKPolylineRenderer`](/documentation/MapKit/MKPolylineRenderer), or [`MKPolygonRenderer`](/documentation/MapKit/MKPolygonRenderer) classes to handle the rendering of that shape on the map.
...
The app doesn’t change the default value of the [`shouldRasterize`](/documentation/MapKit/MKOverlayPathRenderer/shouldRasterize) property, so the standard overlay shapes always remain sharp.
Subclassing any of the provided renderer objects and providing a custom implementation of ``MKOverlayRenderer/draw(_:zoomScale:in:)`, like the `PeakGroundAccelerationOverlayRenderer` class, automatically enables rasterized rendering.
...
For example, the app displays an `MKPolyline` overlay using dashes instead of a solid line, and sets a customized dash pattern using the [`lineDashPattern`](/documentation/MapKit/MKOverlayPathRenderer/lineDashPattern) property of an [`MKPolylineRenderer`](/documentation/MapKit/MKPolylineRenderer).
...
MapKit also provides [`MKGradientPolylineRenderer`](/documentation/MapKit/MKGradientPolylineRenderer) to draw a polyline with a color gradient.
...
Because the app shows each of these overlays using the same color scheme, it groups the individual overlay objects together using an [`MKMultiPolygon`](/documentation/MapKit/MKMultiPolygon) object.
...
Instead, the app returns an [`MKMultiPolygonRenderer`](/documentation/MapKit/MKMultiPolygonRenderer) from the delegate.
...
*Blend modes* relate the content that draws in an overlay to the content that draws behind the overlay.
...
When the map view requests renderer objects from the map delegate for these overlay objects, the app configures the [`blendMode`](/documentation/MapKit/MKOverlayRenderer/blendMode) property with the [`screen`](doc://com.apple.documentation/documentation/CoreGraphics/CGBlendMode/screen) blend mode to lighten the map area outside the park, and the [`colorBurn`](doc://com.apple.documentation/documentation/CoreGraphics/CGBlendMode/colorBurn) blend mode to darken the colors within the park.
...
To create a custom overlay renderer, the app subclasses `MKOverlayRenderer` and implements `draw(_:zoomScale:in:)` to draw the earthquake data into the provided [`CGContext`](doc://com.apple.documentation/documentation/CoreGraphics/CGContext).
...
When the app draws the custom overlay, it uses [`MKMapPoint`](/documentation/MapKit/MKMapPoint) data associated with the overlay to define shapes.
When it needs to convert data between MapKit geometry and Core Graphics geometry, it uses [`point(for:)`](/documentation/MapKit/MKOverlayRenderer/point(for:)).

```swift
let point1Conversion = point(for: coord1.mapPoint)
```

MapKit also provides [`mapRect(for:)`](/documentation/MapKit/MKOverlayRenderer/mapRect(for:)) for converting rectangles between MapKit geometry and Core Graphics geometry.
...
To use a custom bitmap map tile overlay, the app uses [`MKTileOverlay`](/documentation/MapKit/MKTileOverlay) to manage loading the tile data and [`MKTileOverlayRenderer`](/documentation/MapKit/MKTileOverlayRenderer) to render the map tiles.
...
The `CustomLoadingTileOverlay` class in this sample code project implements [`loadTile(at:result:)`](/documentation/MapKit/MKTileOverlay/loadTile(at:result:)) to show how to customize tile-loading behavior for specialized loading needs.

### Use an overlay as an annotation

The `MKOverlay` protocol conforms to the [`MKAnnotation`](/documentation/MapKit/MKAnnotation) protocol.
