...
from ..storage import ROOT, Repository
...
def run(args, corpus):
    if args.command=='catalogs':
        from .catalogs import sync_catalogs
        report=sync_catalogs(corpus.root,args.refresh,emit)
        emit({'catalog_sources':len(report['sources']),'errors':report['errors']})
        if report['errors']:
            raise SystemExit(2)
    elif args.command=='seed':
        from .discover import seed_android,seed_apple,seed_features,seed_huawei,seed_previous
