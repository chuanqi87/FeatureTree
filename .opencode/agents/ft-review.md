...
permission:
  ...
  read:
    '*': deny
    docs/*: allow
    docs-raw/official/snapshots/*/*: allow
    archive/*: deny
...
inputs.source_refs 提供已哈希核对的正文路径；可用 official_sources 与 read 对具体证据复核，不读取其他运行目录。只输出 review payload，不自行修改节点。
