...
* **Compatibility Mode and OpenGL ES Support**: Using WebGPU with [compatibility mode](https://github.com/gpuweb/gpuweb/blob/main/proposals/compatibility-mode.md#webgpu-spec-changes)
  is possible by setting the standardized
  [`featureLevel`](https://gpuweb.github.io/gpuweb/#dom-gpurequestadapteroptions-featurelevel) option to `compatibility` while requesting the
  [`GPUAdapter`](https://developer.android.com/reference/kotlin/androidx/webgpu/GPUAdapter).
...
1. **Start a New Project**: Open Android Studio and
...
2. **Select a template**: Choose the **Empty Activity** template in Android Studio and click **Next**.
   ...
   ![The Android Studio New Project dialog, showing the built-in list of
       activities that Studio will create on your behalf.](https://developer.android.com/develop/ui/views/graphics/webgpu/images/New%20Project.png)
...
3. **Configure your project**:
   ...
   * **Build Configuration Language**: It is recommended to use **Kotlin DSL (build.gradle.kts)** for modern dependency management.
   ![The Android Studio Empty Activity dialog that contains fields to
       ...
       Location, and Minimum SDK.](https://developer.android.com/develop/ui/views/graphics/webgpu/images/Empty%20Activity.png)
...
* Add the `google` repository to `settings.gradle` as described in
...
* **UI Layer**: The UI is built with Jetpack Compose. A WebGPU drawing surface
  is integrated into the Compose hierarchy using
  [`AndroidExternalSurface`](https://developer.android.com/reference/kotlin/androidx/compose/foundation/AndroidExternalSurface.composable#AndroidExternalSurface(androidx.compose.ui.Modifier,kotlin.Boolean,androidx.compose.ui.unit.IntSize,androidx.compose.foundation.AndroidExternalSurfaceZOrder,kotlin.Boolean,kotlin.Function1)).
...
![High-level architecture diagram showing the interaction between the
    ...
    Android application.](https://developer.android.com/develop/ui/views/graphics/webgpu/images/High%20Level%20Architecture%20Graphic.svg)
...
[`AndroidExternalSurface`](https://developer.android.com/reference/kotlin/androidx/compose/foundation/AndroidExternalSurface.composable#AndroidExternalSurface(androidx.compose.ui.Modifier,kotlin.Boolean,androidx.compose.ui.unit.IntSize,androidx.compose.foundation.AndroidExternalSurfaceZOrder,kotlin.Boolean,kotlin.Function1)) to provide a bridge between Compose and your renderer.
...
**Note:** This is where your code needs to release any internal resources it is
using.
...
![A screenshot of an Android phone screen displaying the output of a
    ...
    background.](https://developer.android.com/develop/ui/views/graphics/webgpu/images/Rendered%20Output.png)


**Figure 4.**The rendered output of the sample WebGPU
application showing a red triangle
...
* **WebGpuSurface.kt**: Defines the UI component using `[AndroidExternalSurface](/reference/kotlin/androidx/compose/foundation/AndroidExternalSurface.composable#AndroidExternalSurface(androidx.compose.ui.Modifier,kotlin.Boolean,androidx.compose.ui.unit.IntSize,androidx.compose.foundation.AndroidExternalSurfaceZOrder,kotlin.Boolean,kotlin.Function1))`.
  It manages the `Surface` lifecycle scope, initializing the renderer when the
  surface is ready and cleaning up when it is destroyed.
* **WebGpuRenderer.kt**: Encapsulates all WebGPU-specific logic (Device
  creation, Pipeline setup). It is decoupled from the UI, receiving only the
  `[Surface](/reference/android/view/Surface.html)`
  and dimensions it needs to draw.
...
`[AndroidExternalSurface](/reference/kotlin/androidx/compose/foundation/AndroidExternalSurface.composable#AndroidExternalSurface(androidx.compose.ui.Modifier,kotlin.Boolean,androidx.compose.ui.unit.IntSize,androidx.compose.foundation.AndroidExternalSurfaceZOrder,kotlin.Boolean,kotlin.Function1))` within Jetpack Compose.
...
* **Surface Destruction**: When the user navigates away or the `Surface` is
  destroyed by the system, the lambda block is cancelled. A `finally` block is
  executed, calling `renderer.cleanup()` to prevent memory leaks.
* **Resizing**: If the surface dimensions change, `AndroidExternalSurface` may
  restart the block or directly handle updates depending on configuration, so the
  renderer always writes to a valid buffer.
...
* **Error Scopes:** You can capture specific errors by encapsulating GPU
  commands within
  `[device.pushErrorScope()](/reference/kotlin/androidx/webgpu/GPUDevice#pushErrorScope(kotlin.Int))` and
  `[device.popErrorScope()](https://developer.android.com/reference/kotlin/androidx/webgpu/GPUDevice#popErrorScope(java.util.concurrent.Executor,androidx.webgpu.GPURequestCallback))
  blocks.
...
**Note:** It is recommended to assign .label = "Descriptive Name" to all resources;
this label is integrated into error messages, simplifying the debugging process.
...
* **Avoid Per-Frame Object Creation**: Instantiate pipelines
  ([`GPURenderPipeline`](https://developer.android.com/reference/kotlin/androidx/webgpu/GPURenderPipeline)), bind group layouts, and shader modules once during application setup to maximize reuse.
* **Optimize Buffer Usage**: Update the contents of existing
  `GPUBuffers` via `GPUQueue.writeBuffer` instead of creating new buffers each frame.
